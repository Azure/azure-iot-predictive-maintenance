...
function Build() {
    ...
    If ($?) {
        Write-Host "Building PredictiveMaintenance.sln succeeded"
    } Else {
        throw "Building PredictiveMaintenance.sln failed - See above logs"
    }
}

# Package - see :Package in build.cmd
function Package() {
    &$msbuild ("Web\Web.csproj", "/v:m", "/p:Configuration=$configuration", "/t:Package")
    If (-Not($?)) {
        throw "Packaging Web\Web.csproj failed"
    }
